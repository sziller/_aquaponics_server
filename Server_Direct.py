"""
This is a direct server setup for on router.
FastAPI hints:
...
import urllib
import yaml
from mhmc_routers.aquaponics import aquaponics_router
import importlib
...
from Aquaponics import DataBaseAlchemy as DBAl

print("=" * 66)
print("={:^64}=".format({True: "LIVE SESSION", False: "DEV. SESSION"}[conf.isLIVE]))
print("={:^64}=".format(__name__))
print("=" * 66)
...
fill_in = {"path": conf.PATH_ERROR_MSG, "fn": "Server_SmartHome.py"}
with open(conf.FASTAPI_META_PATH, 'r') as stream:
    try:
        fastapi_data = yaml.safe_load(stream)
        ERROR = parsed_yaml
        print("Loaded error messages from {path} - sais {fn}".format(**fill_in))
    except yaml.YAMLError as exc:
        print(exc)
...
main_DB_session = DBAl.createSession(db_path=conf.DATABASE_NAME, style=conf.DATABASE_STYLE)
print("[PREPARE]: Actions taken for {} version:".format({True: "LIVE", False: " DEV"}[conf.isLIVE]))
prepare.default_users(session_in=main_DB_session)


server = FastAPI(
    openapi_tags=tags_metadata,
    title=conf.ROUTER_INFO[0]["name"].capitalize(),
    version=fastapi_data["version"],
    summary=fastapi_data["summary"],
    description=fastapi_data["description"],
    contact={"name": conf.ROUTER_INFO[0]["name"],
             "email": "szillerke@gmail.com"},
    terms_of_service=fastapi_data["terms"],
    openapi_url=fastapi_data["url"].format(conf.ROUTER_INFO[0]["name"])
    )
